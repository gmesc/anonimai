...
DETECTORS = [
    ...
    ("DATE",
     re.compile(r"(?<!\d)(?:0?[1-9]|[12]\d|3[01])[/.\-]"
                ...
                r"(?:\s+\d{1,2}[:.]\d{2})?(?!\d)"),
     None, True),
    # ------------------------- profilo Svizzera/Ticino -------------------------
    # Stessi tag della tassonomia, identificativi CH: la casella per il numero
    # AVS e' ID_DOC (la tassonomia manda li' ogni numero previdenziale personale),
    # il Cantone e' PROVINCE, il NAP e' ZIPCODE. Sempre attivi: un formato AVS in
    # un documento italiano non esiste, un cantone citato E' una PII geografica.
    ("ID_DOC",
...
def match_custom_terms(text, terms):
    """Entita' dai Termini personali: la lista {value, tag} che l'utente salva in
    prefs.json. Match letterale case-insensitive con confini di parola (stessa
    disciplina del matching PDF: niente "DE" dentro "CORDELLA"); gli spazi del
    termine valgono qualsiasi spaziatura (a-capo dei PDF inclusi).
    source="utente": in fusione vince su tutto — un termine inserito a mano e'
    la dichiarazione piu' esplicita possibile."""
    ents = []
    for t in terms or ():
