...
Endpoint HTTP:
  ...
  DELETE /doc/<id>              butta subito il documento dalla RAM (Pulisci, chiusura
                  della finestra, svuotamento per inattivita'): sempre 204, idempotente
...
Server esposto in rete: PII_AUTH="utente:password" (o --auth) mette una credenziale
  su tutto tranne /health e /assets. Il cifrato lo mette un reverse proxy davanti.
...
DOC_TTL = 10 * 60          # fork: secondi di inattivita' dopo cui un documento cade dalla RAM
                           # anche senza nuovi caricamenti (minimizzazione, art. 6-7 nLPD).
                           # Sceso da 30 a 10 minuti quando l'UI ha iniziato a svuotarsi da
                           # sola dopo 7: all'utente diciamo "dopo 7 minuti non resta niente",
                           # e la frase deve restare vera anche quando il browser muore senza
                           # riuscire a chiamare DELETE /doc/<id>. Un TTL piu' lungo della
                           # promessa fatta a schermo e' una promessa falsa.
...
KEEP_TERMS = detectors_local.load_keep_terms()       # fork: Termini in chiaro
...
def analyze(text, excluded=None, mapping_enabled=True):
    ...
    # fork: Termini in chiaro — l'utente ha dichiarato che questi valori non sono
    # dati personali (i nomi delle scale nei referti: Ritvo, Beck, Wechsler).
    # Si filtra DOPO la fusione, non prima: il confronto deve avvenire sul testo
    # dell'entita' definitiva, non su un candidato che il merge potrebbe allargare.
    kept, cleared = detectors_local.drop_keep_terms(text, kept, KEEP_TERMS)
    ...
    return {
        ...
        # quante occorrenze sono state lasciate in chiaro di proposito, per tag.
        # MAI i valori: quelli restano in prefs.json su questa macchina.
        "cleared_by_tag": dict(sorted(cleared.items(), key=lambda x: -x[1])),
...
# --- fork gmesc/anonimai: credenziale quando il server e' esposto -------------
# Attiva solo se c'e' PII_AUTH (o --auth): su 127.0.0.1 non serve e non compare.
# Aperte comunque: /health e /healthz (la sonda del container non ha credenziali)
# e /assets (icone e fogli di stile: nessun dato). La finestra di login la disegna
# il browser, quindi nell'UI non c'e' una riga in piu'.
AUTH = server_config.load_auth()
...
def _require_auth():
    if not AUTH:
        return None
    p = request.path
    if p in ("/health", "/healthz", "/favicon.ico") or p.startswith("/assets/"):
        return None
    if server_config.check_basic(request.headers.get("Authorization"), AUTH):
        return None
    return app.response_class(
        "Accesso riservato.", status=401,
        headers={"WWW-Authenticate": 'Basic realm="AnonimAI", charset="UTF-8"'},
        mimetype="text/plain")
...
def doc_delete(doc_id):
    """fork: butta subito un documento dalla RAM, senza aspettare il TTL.

    La chiamano «Pulisci», il caricamento di un documento nuovo, lo svuotamento per
    inattivita' e la chiusura della finestra (pagehide, con keepalive). Prima di
    questa rotta il TTL era l'unico modo per liberare la memoria: chi finiva una
    pratica e chiudeva l'app lasciava il PDF nel processo per la mezz'ora buona.

    Sempre 204, anche per un id che non esiste piu': il client non deve distinguere
    «cancellato adesso» da «gia' scaduto», e un 404 lo costringerebbe a gestire un
    errore che non e' un errore. Idempotente per costruzione."""
    with _DOCS_LOCK:
...
def settings_post():
    global EXCLUDED_TAGS, MAPPING_ENABLED, CUSTOM_TERMS, KEEP_TERMS
    ...
    keep = data.get("keep_terms") if "keep_terms" in data else None
    if tags is None and mapping is None and terms is None and keep is None:
        return jsonify({"error": "Niente da salvare: passa excluded_tags, "
                                 "mapping_enabled, custom_terms e/o keep_terms."}), 400
    ...
    if keep is not None:                               # fork: Termini in chiaro
        KEEP_TERMS = detectors_local.save_keep_terms(keep)
...
.dict td.act{width:1%;text-align:right;white-space:nowrap}
.dict td.act .rm{height:var(--tb-h);min-width:var(--tb-min);border:0;background:transparent;
     cursor:pointer;color:var(--muted);font-size:13px;transition:background var(--speed)}
.dict td.act .rm:hover{background:var(--hover);color:var(--ink)}
...
<!-- fork: promemoria del blocco schermo (dopo le Condizioni, con "non ricordarmelo piu'") -->
<div class="cfg-overlay" id="lockOverlay">
  <div class="cfg-card" role="dialog" aria-modal="true" aria-labelledby="lockTitle">
    <h3 id="lockTitle">🔒 <span data-i18n="lock_title">Il computer incustodito</span></h3>
    <div class="cfg-body" data-i18n="lock_body"></div>
    <div class="cfg-btns" style="border-top:1px solid var(--line);margin-top:14px;padding-top:12px">
      <label class="keep" style="flex:1"><input type="checkbox" id="lockOff">
        <span data-i18n="lock_never">Non ricordarmelo piu'</span></label>
      <button class="btn" id="lockOk" onclick="closeLockReminder()" data-i18n="lock_ok">Ho capito</button>
    </div>
  </div>
</div>

<!-- fork: velo di sessione svuotata per inattivita' -->
<div class="cfg-overlay" id="idleOverlay">
  <div class="cfg-card" role="dialog" aria-modal="true" aria-labelledby="idleTitle">
    <h3 id="idleTitle">🧹 <span data-i18n="idle_title">Sessione svuotata</span></h3>
    <div class="cfg-body" data-i18n="idle_body"></div>
    <div class="cfg-btns" style="border-top:1px solid var(--line);margin-top:14px;padding-top:12px">
      <span style="flex:1"></span>
      <button class="btn" onclick="$('idleOverlay').classList.remove('open')"
              data-i18n="idle_ok">Ricomincia</button>
    </div>
  </div>
</div>
...
<div class="cfg-overlay" id="cfgOverlay">
  <div class="cfg-card wide">
    ...
    <div class="tabs" role="tablist">
      ...
      <button class="tab" role="tab" aria-selected="false" id="stHabits"
              onclick="setSettingsTab('habits')" data-i18n="set_tab_habits">Buone abitudini</button>
    ...
    <div class="cfg-body" id="setHabits" style="display:none">
      <div data-i18n="habits_body"></div>
      <div class="cfg-btns" style="margin-top:12px">
        <button class="ghost" onclick="closeConfig();showLockReminder(true)"
                data-i18n="habits_show_reminder">Rivedi il promemoria sul blocco schermo</button>
      </div>
    </div>
...
<div class="cfg-overlay" id="tagsOverlay">
  <div class="cfg-card wide">
    ...
    <div class="ct-head">👁️ <span data-i18n="keep_title">Termini in chiaro</span></div>
    <div class="tg-sub" data-i18n="keep_sub">Valori che <b>non</b> vanno coperti anche se il modello li riconosce: i nomi delle scale nei referti (Ritvo, Beck, Wechsler), una sigla che sembra un nome. <b>Toglie protezione</b>: il Rapporto conta quante volte è stato usato. Anche qui restano salvati su questo computer.</div>
    <div class="ct-list" id="keepList"></div>
...
const T = {
 it:{
  ...
  t_copied:"Copiato — rileggilo prima di incollarlo",
  t_copy_fail:"Copia NON riuscita: negli appunti non c'è questo testo. Apri «Testo da copiare», selezionalo e copialo a mano", t_need_anon:"Prima anonimizza un testo",
  ...
  set_tab_sec:"Sicurezza", set_tab_habits:"Buone abitudini", set_tab_cred:"Crediti", set_tab_terms:"Condizioni", cfg_close:"Chiudi",
  terms_body:"<p><i>Versione 2026-09-04 (rev. 2) · testo completo in <code>TERMS.md</code> nel repository. Bozza: da validare da un avvocato.</i></p><h4>Che cos'è</h4><p>AnonimAI è un <b>software open source di supporto</b> alla <b>pseudonimizzazione</b> di testi e PDF: sostituisce gli identificatori diretti con dei segnaposto. Col dizionario attivo l'operazione è reversibile da chi ha il dizionario; spegnendolo nessuna chiave viene creata. <b>Che il risultato sia un dato anonimo resta una valutazione giuridica</b>, non una proprietà del programma. Gira interamente su questo computer: nessun dato raggiunge gli autori, nessun server, nessuna telemetria, nessun aggiornamento automatico.</p><h4>Chi è responsabile dei dati</h4><p>Chi usa AnonimAI è e resta il <b>titolare del trattamento</b> dei documenti che elabora (nLPD; LPDP per gli enti pubblici ticinesi; GDPR per i dati di persone nell'UE). Gli autori e i contributori <b>non trattano alcun dato</b> e non sono né titolari né responsabili del trattamento.</p><h4>Che cosa non garantisce</h4><ul><li>Il rilevamento si basa su un modello statistico e su regole: <b>può sbagliare</b>, omettere un valore o non riconoscere un'identificazione indiretta.</li><li>Con il dizionario reversibile attivo l'output è una <b>pseudonimizzazione</b>: ancora dato personale per chi ha il dizionario.</li><li>Il software <b>non certifica</b> la conformità di alcun trattamento.</li></ul><h4>Obblighi dell'utente</h4><ul><li><b>Rileggere sempre l'output</b> prima di comunicarlo a terzi; fermarsi davanti agli avvisi di residui o saltati. La <b>supervisione umana non è delegabile</b>.</li><li><b>Rispettare le regole dell'organizzazione per cui si lavora</b>: se operi per uno studio, una scuola, un ente o un'azienda, il titolare del trattamento è quello e valgono le sue direttive. <b>Usare questo software non autorizza nessuna comunicazione</b>: un divieto posto da una direttiva, dal segreto professionale o da una norma resta tale anche con i soli segnaposto.</li><li>Custodire dizionario e Termini personali come l'originale.</li><li>Valutare in proprio comunicazione all'estero (nLPD art. 16-17), valutazione d'impatto (art. 22) e segreto professionale (art. 321 CP).</li><li>Chi espone il server in rete offre un servizio a terzi e ne assume le responsabilità (e il §13 AGPL).</li></ul><h4>Garanzia e responsabilità</h4><p>Il software è fornito <b>«così com'è», senza garanzia</b> (MIT per il sorgente, AGPL-3.0 per i binari). Nei limiti di legge, autori e contributori non rispondono di alcun danno, incluse anonimizzazioni incomplete o errate. L'esclusione non copre ciò che la legge vieta di escludere (dolo e colpa grave, CO art. 100).</p>",
  habits_show_reminder:"Rivedi il promemoria sul blocco schermo",
  lock_title:"Il computer incustodito", lock_never:"Non ricordarmelo più", lock_ok:"Ho capito",
  lock_body:"<p>Se il tuo computer può restare <b>incustodito per più di 5 minuti</b> in un luogo dove passano persone che non hanno diritto di vedere i tuoi dati — la segreteria, lo studio la sera, l'ufficio durante le pulizie — imposta il <b>blocco automatico dello schermo</b> a 5 minuti o meno, con password al risveglio.</p><ul><li><b>Windows</b>: Impostazioni → Account → Opzioni di accesso → <i>Se sei stato assente, quando Windows deve richiedere l'accesso?</i></li><li><b>macOS</b>: Impostazioni di sistema → Schermata di blocco → <i>Richiedi password dopo l'avvio del salvaschermo</i>.</li><li><b>Linux (GNOME)</b>: Impostazioni → Privacy → Blocco schermo.</li><li>E quando ti alzi, senza aspettare il timer: <code>Win+L</code> su Windows, <code>⌃⌘Q</code> su macOS.</li></ul><p>AnonimAI fa la sua parte: dopo <b>7 minuti</b> senza attività svuota la sessione e sul disco non lascia niente. Il blocco dello schermo però lo mette il sistema operativo — nessun programma può imporlo al posto tuo, e uno che dicesse di farlo ti darebbe una sicurezza finta.</p><p>Se lavori per un ente, uno studio o un'azienda, guarda che cosa dice la loro <b>direttiva informatica</b>: quasi sempre il blocco schermo è già obbligatorio, e applicarlo è responsabilità tua. Le altre abitudini che contano sono in ⚙️ → <b>Buone abitudini</b>.</p>",
  idle_title:"Sessione svuotata", idle_ok:"Ricomincia",
  idle_body:"<p>Sono passati <b>7 minuti</b> senza attività: documento, testo, risultato e dizionario di sessione sono stati <b>cancellati</b> — qui e nella memoria del server.</p><p>Quello che avevi già scaricato (PDF anonimizzato, dizionario, rapporto) è al sicuro nella tua cartella dei download. Se avevi attivato «💾 ricorda», il dizionario è ancora qui.</p><p>È lo stesso che fa <b>Pulisci</b>, fatto da solo: così il computer può restare acceso senza restare pieno.</p>",
  t_idle_soon:"Fra un minuto la sessione si svuota da sola: scarica quello che ti serve.",
  map_keep_confirm:"Il dizionario verrà salvato su QUESTO computer, in chiaro, nel profilo dell'app.\n\nContiene tutti i valori veri (nomi, indirizzi, numeri): vale quanto il documento originale, e chiunque acceda a questo computer può leggerlo.\n\nAttivare «ricorda tra le sessioni»?",
  habits_body:"<p><i>Queste abitudini non riguardano l'anonimizzazione: riguardano tutto quello che le sta attorno. Servono a non trovarti nella posizione di chi ha lasciato uscire dei dati per disattenzione — davanti a una contestazione, la differenza fra un errore e una colpa sta quasi sempre qui. Testo informativo, non un parere legale.</i></p><h4>🔒 Il computer incustodito</h4><ul><li><b>Blocco automatico dello schermo entro 5 minuti</b>, con password al risveglio. È la misura che rende inutili tutte le altre se manca.</li><li><b>Blocca quando ti alzi</b>, anche per due minuti: <code>Win+L</code>, <code>⌃⌘Q</code>. Diventa un gesto in una settimana.</li><li><b>Un account per persona</b>, mai «l'account della segreteria»: se qualcosa succede da quel computer deve essere chiaro chi era collegato.</li><li><b>Chi entra a pulire, riparare, fare manutenzione</b>: clausola di riservatezza nel contratto, e per quanto possibile non da solo negli spazi dove ci sono i dati.</li><li><b>A fine giornata niente in vista</b>, né a schermo né sulla scrivania: lo schermo acceso è la versione moderna del fascicolo lasciato aperto.</li><li>Questa app fa la sua parte: si svuota da sola dopo <b>7 minuti</b> e non scrive niente su disco.</li></ul><h4>☁️ Cloud e dispositivi personali</h4><ul><li>Telefono e computer di casa <b>sincronizzano da soli</b>: foto, schermate, cartelle, appunti. La foto a un documento o la schermata di questa app finisce su un servizio estero senza che tu l'abbia deciso. È il modo più comune in cui i dati escono senza che nessuno se ne accorga.</li><li><b>Niente documenti di lavoro sui dispositivi personali.</b> Se proprio devi, spegni la sincronizzazione per quella cartella e cancella appena hai finito.</li><li><b>Niente posta personale</b> per i documenti di lavoro, e nessun inoltro automatico verso una casella privata.</li><li><b>Diffida dei servizi che sembrano innocui</b>: traduttori, correttori, convertitori di PDF, trascrizione automatica, assistenti dentro la tastiera del telefono. Mandano tutti il testo a un server. Se il testo non è anonimizzato, l'hai comunicato a un terzo.</li><li>La cartella dove tieni le pratiche <b>non va sincronizzata</b> con un servizio personale.</li></ul><h4>🎣 Programmi che rubano le password</h4><ul><li>Un <b>ladro di credenziali</b> (in inglese <i>info-stealer</i>) è un programma che entra con un download — un convertitore gratuito, un programma craccato, un finto aggiornamento, un'estensione del browser, un allegato — e porta via in silenzio le password salvate nel browser, i cookie delle sessioni aperte e i file recenti. Nessuno forza niente: poi entrano con le tue credenziali, e sembra te.</li><li><b>Non salvare le password di lavoro nel browser.</b> Usa un gestore di password: una parola d'accesso sola, lunga, che sai a memoria.</li><li><b>Verifica in due passaggi</b> ovunque sia disponibile: è ciò che regge quando la password è già stata rubata.</li><li><b>Nessun programma non autorizzato</b>, nessuna estensione «utile» installata al volo, niente software craccato: è la porta d'ingresso più battuta.</li><li><b>Aggiornamenti attivi</b> per sistema e browser. Un browser separato per il lavoro tiene i cookie lontani dalla navigazione personale.</li></ul><h4>✉️ Posta e messaggi</h4><ul><li>Il mittente si <b>controlla</b>, non si riconosce dal nome mostrato: quel nome lo scrive chi manda il messaggio.</li><li><b>Nessun allegato inatteso</b>, nemmeno da un indirizzo conosciuto: gli account rubati mandano posta vera.</li><li><b>Nessun link</b> per «riattivare l'account», «verificare la casella», «firmare il documento». Al sito ci si va digitando l'indirizzo.</li><li>Nel dubbio si <b>telefona</b>, al numero che hai già — non a quello scritto nel messaggio.</li></ul><h4>🗂️ I file che escono da qui</h4><ul><li>Il <b>dizionario</b> vale quanto il documento originale: mai nella stessa cartella del file anonimizzato, mai nello stesso allegato, mai in una chat.</li><li>PDF e rapporti scaricati finiscono nella cartella dei download: <b>spostali</b> dove tieni la pratica e svuota i download.</li><li>A pratica chiusa cancella dizionari e copie di lavoro. E tieni il <b>disco cifrato</b> (FileVault su macOS, BitLocker su Windows): protegge il computer rubato, non lo schermo acceso.</li></ul><h4>⚖️ Perché conviene, in concreto</h4><ul><li>La legge federale prevede <b>multe fino a 250'000 franchi a carico della persona</b> che viola intenzionalmente certi obblighi — della persona, non dell'ente.</li><li>Per medici, avvocati, notai e i loro collaboratori la violazione del segreto professionale è <b>materia penale</b> (art. 321 CP).</li><li>Per chi lavora in un ente pubblico si aggiungono le conseguenze disciplinari previste dall'ente.</li><li>Nessuna di queste abitudini richiede un budget o un permesso: è questo che le rende difficili da giustificare quando mancano.</li><li><i>Bozza informativa, da far leggere a un consulente prima di appoggiarci una decisione d'ente.</i></li></ul>",
  terms_first_title:"Prima di iniziare", terms_ok:"Ho capito",
  terms_first_body:"<p>AnonimAI è uno <b>strumento di supporto</b> alla <b>pseudonimizzazione</b>: gira su questo computer e nessun dato raggiunge gli autori. Quattro cose da sapere prima di cominciare.</p><ul><li><b>Prima di incollare in un chatbot, la verifica umana è obbligatoria.</b> Non consigliata: obbligatoria. Il rilevamento è statistico e <b>può sbagliare</b> — un nome fuori posto, una sigla scambiata per un'altra cosa — e l'invio a un servizio esterno <b>non si può annullare</b>: quello che è uscito è uscito. <b>Rileggi il testo con i segnaposto</b> subito prima di incollarlo in ChatGPT o in qualunque altro strumento, e fermati quando l'app ti avvisa che qualcosa è rimasto in chiaro. È l'ultimo momento in cui un errore si ferma ancora, e nessun controllo automatico sostituisce quella lettura: nemmeno questo.</li><li><b>Rispetta le regole di chi risponde dei dati.</b> Se lavori per uno studio, una scuola, un ente o un'azienda, <b>il titolare del trattamento è quello</b>, e valgono le sue direttive: quali documenti possono uscire, verso quali servizi esterni, con quale autorizzazione. <b>Anonimizzare non autorizza</b>: se una direttiva interna, il segreto professionale o una norma vietano quell'invio, il divieto resta anche con i segnaposto.</li><li><b>Quello che esce resta un dato personale.</b> Con il dizionario attivo è una <b>pseudonimizzazione</b>, reversibile da chiunque abbia il dizionario. Spegnendolo nessuna chiave viene creata, ma se il testo sia davvero anonimo lo decide una valutazione, non il programma: una vicenda raccontata per esteso identifica la persona anche senza il suo nome.</li><li><b>Nessuna garanzia</b> (MIT / AGPL-3.0): il software non certifica la conformità di nessun trattamento. La responsabilità resta di chi tratta i dati — tu, o l'ente per cui lavori.</li></ul><p>Testo completo in ⚙️ Impostazioni → Condizioni. Le abitudini che riducono i rischi attorno al computer: ⚙️ → Buone abitudini.</p>",
  ...
  cred_body:"<h4>Il progetto originale, e che cosa è stato modificato</h4><p>AnonimAI è una <b>versione modificata</b> di <b>rizzo-pii</b>, di <b>Simone Rizzo</b> — Rizzo AI Academy (licenza MIT). Da lì vengono il modello, il dataset, la pipeline di addestramento e la prima forma dell'app locale.</p><p>Le modifiche sono di <b>Giacomo Meschini</b>, agosto-settembre 2026: profilo di riconoscimento svizzero e ticinese, redazione vera dei PDF con OCR e riquadri manuali, misure di protezione dei dati (nessuna chiamata di rete, niente su disco, svuotamento della sessione, credenziale per il server esposto), condizioni d'uso e documentazione di conformità. L'elenco datato è in <code>NOTICE.md</code>; l'originale non modificato è nel repository di Rizzo AI Academy, che non risponde di questo fork.</p><div class=\"kv\"><b>Repository</b><span><a href=\"https://github.com/Rizzo-AI-Academy/rizzo-pii\" target=\"_blank\" rel=\"noopener\">github.com/Rizzo-AI-Academy/rizzo-pii ↗</a></span></div><div class=\"kv\"><b>Sito</b><span><a href=\"https://www.rizzoaiacademy.com\" target=\"_blank\" rel=\"noopener\">www.rizzoaiacademy.com ↗</a></span></div><div class=\"kv\"><b>Modello</b><span><span id=\"credModel\">rizzo-pii:0.3B</span></span></div><div class=\"kv\"><b>Pesi</b><span><a href=\"https://huggingface.co/rizzoaiacademy/rizzo-pii-0.3B\" target=\"_blank\" rel=\"noopener\">rizzoaiacademy/rizzo-pii-0.3B ↗</a> · base <a href=\"https://huggingface.co/jhu-clsp/mmBERT-base\" target=\"_blank\" rel=\"noopener\">jhu-clsp/mmBERT-base ↗</a></span></div><div class=\"kv\"><b>Dataset</b><span><a href=\"https://huggingface.co/datasets/rizzoaiacademy/anonimizzazione-testi-italiano\" target=\"_blank\" rel=\"noopener\">rizzoaiacademy/anonimizzazione-testi-italiano ↗</a></span></div><h4>Licenze</h4><ul><li><b>Codice sorgente</b>: MIT — © 2026 Simone Rizzo (Rizzo AI Academy) e © 2026 Giacomo Meschini.</li><li><b>Binari distribuiti</b>: AGPL-3.0, perché incorporano PyMuPDF (AGPL-3.0 o licenza commerciale Artifex).</li><li><b>Icone</b>: OpenMoji — CC BY-SA 4.0.</li><li><b>Documenti di analisi</b> (conformità, quadro per settori, modelli): CC BY-SA 4.0 — riusabili citando la fonte.</li><li><b>Librerie</b>: PyTorch · Transformers · tokenizers · safetensors (Apache-2.0) · Flask (BSD-3-Clause) · Tauri (MIT o Apache-2.0) · Tesseract, dentro PyMuPDF (Apache-2.0).</li></ul><p>L'elenco completo e verificabile è in <code>THIRD_PARTY_LICENSES.md</code> nel repository.</p>",
  ...
  keep_title:"Termini in chiaro", keep_anytag:"ovunque",
  keep_sub:"Valori che <b>non</b> vanno coperti anche se il modello li riconosce: i nomi delle scale nei referti (Ritvo, Beck, Wechsler), una sigla che sembra un nome. <b>Toglie protezione</b>: il Rapporto conta quante volte è stato usato. Restano salvati su questo computer.",
  keep_del:"Togli dalla lista: questo valore tornera' a essere coperto",
  keep_add:"Lascia in chiaro: questo valore non e' un dato personale",
  keep_confirm:"«{v}» non verra' piu' coperto, in questo e nei prossimi documenti, quando compare come {t}.\n\nUsalo per cio' che NON e' un dato personale: il nome di una scala, di un test, di un metodo. La regola resta su questo computer e si toglie dalla scheda 🏷️.\n\nLasciare «{v}» in chiaro?",
  keep_done:"«{v}» resta in chiaro. Rileggi il risultato.",
  st_clear:"lasciati in chiaro",
 ...
 en:{
  ...
  t_copied:"Copied — re-read it before you paste",
  t_copy_fail:"Copy FAILED: this text is not in your clipboard. Open “Text to copy”, select it and copy it by hand", t_need_anon:"Anonymize a text first",
  ...
  set_tab_sec:"Security", set_tab_habits:"Good habits", set_tab_cred:"Credits", set_tab_terms:"Terms", cfg_close:"Close",
  terms_body:"<p><i>Version 2026-09-04 (rev. 2) · full text in <code>TERMS.md</code> in the repository. Draft: to be validated by a lawyer.</i></p><h4>What it is</h4><p>AnonimAI is open-source <b>support software</b> for <b>pseudonymising</b> text and PDFs: it replaces direct identifiers with placeholders. With the dictionary on the operation is reversible by whoever holds it; switch it off and no key is created. <b>Whether the result qualifies as anonymous data remains a legal assessment</b>, not a property of the program. It runs entirely on this computer: no data reaches the authors, no server, no telemetry, no automatic updates.</p><h4>Who is responsible for the data</h4><p>Whoever uses AnonimAI is and remains the <b>data controller</b> for the documents processed (Swiss FADP; cantonal LPDP for Ticino public bodies; GDPR for EU data subjects). Authors and contributors <b>process no data</b> and are neither controllers nor processors.</p><h4>What it does not guarantee</h4><ul><li>Detection relies on a statistical model and rules: it <b>can be wrong</b>, miss a value, or fail to see indirect identification.</li><li>With the reversible dictionary on, the output is a <b>pseudonymisation</b>: still personal data for whoever holds the dictionary.</li><li>The software <b>does not certify</b> the compliance of any processing.</li></ul><h4>User obligations</h4><ul><li><b>Always re-read the output</b> before sharing it; stop on residual/skipped warnings. <b>Human review cannot be delegated.</b></li><li><b>Follow the rules of the organisation you work for</b>: if you act for a firm, a school, a public body or a company, that organisation is the controller and its policies apply. <b>Using this software authorises no disclosure</b>: a ban set by a policy, by professional secrecy or by law still stands, placeholders or not.</li><li>Guard the dictionary and Personal terms like the original.</li><li>Assess on your own cross-border disclosure (FADP art. 16-17), impact assessment (art. 22) and professional secrecy (art. 321 SCC).</li><li>Whoever exposes the server on a network provides a service to others and takes on that responsibility (and AGPL §13).</li></ul><h4>Warranty and liability</h4><p>The software is provided <b>“as is”, without warranty</b> (MIT for the source, AGPL-3.0 for the binaries). To the extent permitted by law, authors and contributors are not liable for any damage, including incomplete or wrong anonymisation. The exclusion does not cover what the law forbids excluding (intent and gross negligence, CO art. 100).</p>",
  habits_show_reminder:"Show the screen-lock reminder again",
  lock_title:"The unattended computer", lock_never:"Do not remind me again", lock_ok:"Got it",
  lock_body:"<p>If your computer can be left <b>unattended for more than 5 minutes</b> somewhere people walk through who have no right to see your data — the front desk, the office in the evening, the room while it is being cleaned — set the <b>automatic screen lock</b> to 5 minutes or less, with a password on wake.</p><ul><li><b>Windows</b>: Settings → Accounts → Sign-in options → <i>If you have been away, when should Windows require you to sign in again?</i></li><li><b>macOS</b>: System Settings → Lock Screen → <i>Require password after screen saver begins</i>.</li><li><b>Linux (GNOME)</b>: Settings → Privacy → Screen Lock.</li><li>And when you stand up, without waiting for the timer: <code>Win+L</code> on Windows, <code>⌃⌘Q</code> on macOS.</li></ul><p>AnonimAI does its part: after <b>7 minutes</b> of inactivity it wipes the session, and it writes nothing to disk. The screen lock itself belongs to the operating system — no program can set it for you, and one that claimed to would be giving you fake security.</p><p>If you work for an organisation, check its <b>IT policy</b>: the screen lock is almost always mandatory already, and applying it is your responsibility. The other habits that matter are in ⚙️ → <b>Good habits</b>.</p>",
  idle_title:"Session wiped", idle_ok:"Start again",
  idle_body:"<p><b>7 minutes</b> went by with no activity: document, text, result and session dictionary have been <b>deleted</b> — here and in the server's memory.</p><p>Whatever you had already downloaded (anonymised PDF, dictionary, report) is safe in your downloads folder. If you had enabled “💾 remember”, the dictionary is still here.</p><p>It is what <b>Clear</b> does, done on its own: the computer can stay on without staying full.</p>",
  t_idle_soon:"In one minute the session wipes itself: download what you need.",
  map_keep_confirm:"The dictionary will be saved on THIS computer, in the clear, in the app profile.\n\nIt holds every real value (names, addresses, numbers): it is worth as much as the original document, and anyone with access to this computer can read it.\n\nEnable “remember across sessions”?",
  habits_body:"<p><i>These habits are not about anonymisation: they are about everything around it. They keep you out of the position of someone who let data escape through carelessness — when something is contested, the difference between a mistake and negligence usually sits right here. Informative text, not legal advice.</i></p><h4>🔒 The unattended computer</h4><ul><li><b>Automatic screen lock within 5 minutes</b>, password on wake. Without it, every other measure is decoration.</li><li><b>Lock when you stand up</b>, even for two minutes: <code>Win+L</code>, <code>⌃⌘Q</code>. It becomes a reflex in a week.</li><li><b>One account per person</b>, never “the front-desk account”: if something happens from that computer, it must be clear who was signed in.</li><li><b>Cleaners, repair and maintenance staff</b>: a confidentiality clause in the contract, and as far as possible not alone in the rooms where the data is.</li><li><b>Nothing on display at the end of the day</b>, on screen or on the desk: a live screen is the modern version of an open file left out.</li><li>This app does its part: it wipes itself after <b>7 minutes</b> and writes nothing to disk.</li></ul><h4>☁️ Cloud services and personal devices</h4><ul><li>Your phone and your home computer <b>sync by themselves</b>: photos, screenshots, folders, notes. A photo of a document or a screenshot of this app ends up on a foreign service without you deciding anything. It is the most common way data leaves unnoticed.</li><li><b>No work documents on personal devices.</b> If you really must, turn sync off for that folder and delete as soon as you are done.</li><li><b>No personal mail</b> for work documents, and no automatic forwarding to a private mailbox.</li><li><b>Beware of harmless-looking services</b>: translators, proofreaders, PDF converters, automatic transcription, assistants built into the phone keyboard. They all send the text to a server. If the text is not anonymised, you have disclosed it to a third party.</li><li>The folder holding your cases <b>must not be synced</b> with a personal service.</li></ul><h4>🎣 Programs that steal passwords</h4><ul><li>An <b>info-stealer</b> is a program that arrives with a download — a free converter, a cracked program, a fake update, a browser extension, an attachment — and quietly takes the passwords saved in your browser, the cookies of your open sessions and your recent files. Nothing is forced: afterwards they sign in with your credentials, and it looks like you.</li><li><b>Do not save work passwords in the browser.</b> Use a password manager: one long passphrase you know by heart.</li><li><b>Two-step verification</b> wherever it exists: it is what holds when the password is already stolen.</li><li><b>No unauthorised programs</b>, no “useful” extension installed on the spot, no cracked software: it is the most travelled way in.</li><li><b>Updates on</b> for the system and the browser. A separate browser for work keeps work cookies away from personal browsing.</li></ul><h4>✉️ Mail and messages</h4><ul><li>The sender is <b>checked</b>, not recognised by the displayed name: that name is written by whoever sends the message.</li><li><b>No unexpected attachment</b>, not even from a known address: stolen accounts send real mail.</li><li><b>No link</b> to “reactivate your account”, “verify your mailbox”, “sign the document”. You reach a site by typing its address.</li><li>When in doubt, <b>phone</b> — the number you already have, not the one in the message.</li></ul><h4>🗂️ The files that leave this app</h4><ul><li>The <b>dictionary</b> is worth as much as the original document: never in the same folder as the anonymised file, never in the same attachment, never in a chat.</li><li>Downloaded PDFs and reports land in your downloads folder: <b>move them</b> to the case folder and empty downloads.</li><li>When a case closes, delete dictionaries and working copies. And keep the <b>disk encrypted</b> (FileVault, BitLocker): it protects a stolen computer, not a live screen.</li></ul><h4>⚖️ Why it pays, concretely</h4><ul><li>Swiss federal law provides for <b>fines up to CHF 250'000 on the individual</b> who intentionally breaches certain duties — the individual, not the organisation.</li><li>For physicians, lawyers, notaries and their staff, breaking professional secrecy is a <b>criminal matter</b> (art. 321 of the Criminal Code).</li><li>For public-sector staff, the employer's disciplinary consequences come on top.</li><li>None of these habits needs a budget or an approval: that is exactly what makes them hard to justify when they are missing.</li><li><i>Informative draft: have a consultant read it before basing an organisational decision on it.</i></li></ul>",
  terms_first_title:"Before you start", terms_ok:"Got it",
  terms_first_body:"<p>AnonimAI is a <b>support tool</b> for <b>pseudonymisation</b>: it runs on this computer and no data reaches the authors. Four things to know before you start.</p><ul><li><b>Human review is mandatory before you paste into a chatbot.</b> Not advisable: mandatory. Detection is statistical and <b>can be wrong</b> — a name missed, an abbreviation mistaken for something else — and sending text to an external service <b>cannot be undone</b>: what left has left. <b>Re-read the placeholder text</b> right before pasting it into ChatGPT or any other tool, and stop when the app warns you that something was left in the clear. It is the last moment an error can still be caught, and no automatic check replaces that reading: not even this one.</li><li><b>Follow the rules of whoever answers for the data.</b> If you work for a firm, a school, a public body or a company, <b>that organisation is the data controller</b>, and its rules apply: which documents may leave, to which external services, with whose authorisation. <b>Anonymising does not authorise</b>: if an internal directive, professional secrecy or a legal duty forbids that transfer, the ban stands even with placeholders.</li><li><b>What comes out is still personal data.</b> With the dictionary on it is <b>pseudonymisation</b>, reversible by anyone holding the dictionary. Switch it off and no key is created, but whether the text is truly anonymous is a legal assessment, not something the program can decide: a story told in detail identifies the person even without a name.</li><li><b>No warranty</b> (MIT / AGPL-3.0): the software certifies the compliance of no processing. Responsibility stays with whoever processes the data — you, or the organisation you work for.</li></ul><p>Full text in ⚙️ Settings → Terms. The habits that reduce the risks around the computer: ⚙️ → Good habits.</p>",
  ...
  cred_body:"<h4>The original project, and what was modified</h4><p>AnonimAI is a <b>modified version</b> of <b>rizzo-pii</b>, by <b>Simone Rizzo</b> — Rizzo AI Academy (MIT licence). The model, the dataset, the training pipeline and the first shape of the local app come from there.</p><p>The modifications are by <b>Giacomo Meschini</b>, August–September 2026: the Swiss/Ticino recognition profile, real PDF redaction with OCR and manual boxes, data-protection measures (no network calls, nothing on disk, session wipe, credential for an exposed server), terms of use and compliance documentation. The dated list is in <code>NOTICE.md</code>; the unmodified original lives in the Rizzo AI Academy repository, which does not answer for this fork.</p><div class=\"kv\"><b>Repository</b><span><a href=\"https://github.com/Rizzo-AI-Academy/rizzo-pii\" target=\"_blank\" rel=\"noopener\">github.com/Rizzo-AI-Academy/rizzo-pii ↗</a></span></div><div class=\"kv\"><b>Website</b><span><a href=\"https://www.rizzoaiacademy.com\" target=\"_blank\" rel=\"noopener\">www.rizzoaiacademy.com ↗</a></span></div><div class=\"kv\"><b>Model</b><span><span id=\"credModel\">rizzo-pii:0.3B</span></span></div><div class=\"kv\"><b>Weights</b><span><a href=\"https://huggingface.co/rizzoaiacademy/rizzo-pii-0.3B\" target=\"_blank\" rel=\"noopener\">rizzoaiacademy/rizzo-pii-0.3B ↗</a> · backbone <a href=\"https://huggingface.co/jhu-clsp/mmBERT-base\" target=\"_blank\" rel=\"noopener\">jhu-clsp/mmBERT-base ↗</a></span></div><div class=\"kv\"><b>Dataset</b><span><a href=\"https://huggingface.co/datasets/rizzoaiacademy/anonimizzazione-testi-italiano\" target=\"_blank\" rel=\"noopener\">rizzoaiacademy/anonimizzazione-testi-italiano ↗</a></span></div><h4>Licences</h4><ul><li><b>Source code</b>: MIT — © 2026 Simone Rizzo (Rizzo AI Academy) and © 2026 Giacomo Meschini.</li><li><b>Released binaries</b>: AGPL-3.0, because they bundle PyMuPDF (AGPL-3.0 or Artifex commercial licence).</li><li><b>Icons</b>: OpenMoji — CC BY-SA 4.0.</li><li><b>Analysis documents</b> (compliance map, sector report, templates): CC BY-SA 4.0 — reusable with credit.</li><li><b>Libraries</b>: PyTorch · Transformers · tokenizers · safetensors (Apache-2.0) · Flask (BSD-3-Clause) · Tauri (MIT or Apache-2.0) · Tesseract, inside PyMuPDF (Apache-2.0).</li></ul><p>The full, verifiable list is in <code>THIRD_PARTY_LICENSES.md</code> in the repository.</p>",
  ...
  keep_title:"Terms left in the clear", keep_anytag:"anywhere",
  keep_sub:"Values that must <b>not</b> be covered even when the model recognises them: the names of scales in a report (Ritvo, Beck, Wechsler), an abbreviation that looks like a name. <b>It removes protection</b>: the Report counts how often it was used. They too are saved on this computer.",
  keep_del:"Remove from the list: this value will be covered again",
  keep_add:"Leave in the clear: this value is not personal data",
  keep_confirm:"“{v}” will no longer be covered, in this and in future documents, when it appears as {t}.\n\nUse it for what is NOT personal data: the name of a scale, a test, a method. The rule stays on this computer and can be removed from the 🏷️ panel.\n\nLeave “{v}” in the clear?",
  keep_done:"“{v}” stays in the clear. Re-read the result.",
  st_clear:"left in the clear",
...
function boxesChanged(){
  dropDoc(OUT_DOC);OUT_DOC=null;$('pdfOutView').innerHTML='';   // il PDF censurato va rifatto
...
async function onFile(f){
  ...
  dropDoc(SRC_DOC);dropDoc(OUT_DOC);OUT_DOC=null;   // fork: il documento di prima non serve piu'
...
function render(){
  ...
  for(const ph of keys){const lab=ph.slice(1,ph.lastIndexOf('_'));const c=colors(lab);
    ...
    tr.innerHTML=`<td class="k" style="color:${c.tx}">${ph}</td>`+
      ...
      `<td class="act"><button class="rm" data-i18n-title="keep_add">👁️</button></td>`;
    // fork: "lascia in chiaro" — il valore entra nei Termini in chiaro (permanenti)
    // e il testo si rianonimizza. Toglie protezione: si conferma, e il Rapporto
    // conta quante occorrenze sono state scoperte di proposito.
    tr.querySelector('.rm').onclick=()=>keepInClear(d.mapping[ph],lab);
    rows.appendChild(tr);}
...
$('copy').onclick=()=>{if(!DATA){toast(tt('t_need_anon'),false);return;}
  navigator.clipboard.writeText(DATA.anonymized_text)
    .then(()=>toast(tt('t_copied')))
    .catch(()=>toast(tt('t_copy_fail'),false,9000));};
...
$('dlrep').onclick=async()=>{
  ...
  const rep={
    ...
    // fork: quante occorrenze sono state lasciate in chiaro di PROPOSITO, per tag.
    // Mai i valori: il Rapporto si archivia con la pratica e non deve contenerne.
    cleared_on_purpose:{count:DATA.n_cleared||0,by_tag:DATA.cleared_by_tag||{}},
    keep_terms_count:(TAGS_META.keep_terms||[]).length,
...
$('rcopy').onclick=()=>{const o=$('rout');if(!o._raw){toast(tt('t_nothing_copy'),false);return;}
  navigator.clipboard.writeText(o._raw)
    .then(()=>toast(tt('t_restored_copied')))
    .catch(()=>toast(tt('t_copy_fail'),false,9000));};
...
/* ---- input helpers ---- */
$('go').onclick=run;
/* ---- fork: Termini in chiaro -------------------------------------------------
   Lo specchio dei Termini personali: valori che NON sono dati personali e vanno
   lasciati leggibili. Il caso che li ha fatti nascere: nei referti le scale
   portano il nome di chi le ha scritte (RAADS-R di Ritvo, Beck, Wechsler), il
   modello li etichetta come persone — giustamente — ma coprirli rende il testo
   incomprensibile a chi lo deve leggere.
   La lista e' PERMANENTE (prefs.json) perche' un nome di scala vale per tutti i
   documenti dello studio, non per uno solo; si toglie dalla scheda 🏷️ con un clic. */
async function keepInClear(valore,tag){
  if(!confirm(tt('keep_confirm').replaceAll('{v}',valore).replaceAll('{t}',tag)))return;
  try{
    const r=await fetch('/settings');const d=await r.json();
    const lista=d.keep_terms||[];
    if(!lista.some(k=>k.value.toLowerCase()===valore.toLowerCase()))
      lista.push({value:valore,tags:[tag]});
    const w=await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({keep_terms:lista})});
    if(!w.ok)throw new Error();
    TAGS_LOADED=false;                      // la scheda tag deve rileggere la lista
    toast(tt('keep_done').replaceAll('{v}',valore));
    if($('src').value.trim()||$('pdf').files.length)run();   // rianonimizza con la regola nuova
  }catch(e){toast(tt('t_error'),false);}
}
...
/* ---- fork: svuotare la sessione ------------------------------------------
   ...
   nella RAM del server finche' non glielo si dice (DELETE /doc/<id>), e prima di
   quella rotta l'unico modo di liberarlo era aspettare il TTL.
   keepSaved: lo svuotamento automatico NON tocca il dizionario che l'utente ha
   chiesto di conservare con «ricorda» — quello e' un salvataggio voluto, e
   cancellarlo per un timer sarebbe perdita di dati non richiesta. «Pulisci» lo
   cancella lo stesso: e' il gesto che dice «ho finito». */
function dropDoc(d){
  if(!d||!d.doc_id)return;
  // keepalive: la richiesta deve partire anche mentre la finestra si sta chiudendo
  try{fetch('/doc/'+encodeURIComponent(d.doc_id),{method:'DELETE',keepalive:true});}catch{}
}
function wipeSession(keepSaved){
  dropDoc(SRC_DOC);dropDoc(OUT_DOC);
  ...
  if(!(keepSaved&&PERSIST())){
    ...
    MAP={};localStorage.removeItem('pii_map');sessionStorage.removeItem('pii_map');
...
$('clear').onclick=()=>wipeSession(false);
...
function setSettingsTab(w){
  for(const [id,pane] of [['stServer','setServer'],['stHow','setHow'],
                          ['stSec','setSec'],['stHabits','setHabits'],
                          ['stCred','setCred'],['stTerms','setTerms']]){
...
function setPersist(on){
  // fork: accendere «ricorda» scrive su disco, in chiaro, tutte le PII del documento.
  // Un clic distratto non basta: su un computer di segreteria condiviso il collega del
  // turno dopo troverebbe i nomi dell'ultima pratica. Spegnerlo non chiede niente.
  if(on&&!confirm(tt('map_keep_confirm'))){$('mapPersist').checked=false;return;}
...
async function openTags(){
  ...
  KEEP=(TAGS_META.keep_terms||[]).map(k=>({value:k.value,tags:(k.tags||[]).slice()}));
  renderTags();
  renderTerms();
  renderKeep();
...
/* fork: i Termini in chiaro si aggiungono dalla tabella del dizionario (dove si
   vede il valore nel suo contesto); qui si guardano e si tolgono. */
let KEEP=[];
function renderKeep(){
  const list=$('keepList');list.innerHTML='';
  for(const [i,k] of KEEP.entries()){
    const tag=(k.tags&&k.tags[0])||'';
    const c=colors(tag||'ORG');
    const row=document.createElement('div');row.className='ct-row';
    row.innerHTML=`<span class="nm" style="color:${c.tx}">${escapeHtml(tag||tt('keep_anytag'))}</span>`+
      `<span class="val">${escapeHtml(k.value)}</span>`+
      `<button class="rm" title="${tt('keep_del')}">✕</button>`;
    row.querySelector('.rm').onclick=()=>{KEEP.splice(i,1);renderKeep();};
    list.appendChild(row);
  }
}
async function saveTags(){
  try{
    const r=await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({excluded_tags:[...EXCL],custom_terms:TERMS,keep_terms:KEEP})});
    const d=await r.json();
    if(!r.ok)throw new Error(d.error||'');
    EXCL=new Set(d.excluded_tags||[]);
    TAGS_META.custom_terms=d.custom_terms||[];
    TAGS_META.keep_terms=d.keep_terms||[];
    toast(tt('tg_saved'));
...
const TERMS_VERSION='2026-09-04r2';   // testo rinforzato: supervisione umana e regole dell'ente
...
function ackTerms(){try{localStorage.setItem('pii_terms_ack',TERMS_VERSION);}catch{}
  $('termsOverlay').classList.remove('open');showLockReminder(false);}

/* ---- fork: promemoria del blocco schermo ----------------------------------
   Il blocco automatico lo imposta il sistema operativo, non quest'app: un
   programma che provasse a metterlo darebbe una sicurezza finta. Quello che
   possiamo fare e' ricordarlo a chi lavora dove il computer resta incustodito
   e passa gente che non ha diritto di vedere quei dati — la stanza della
   segreteria, lo studio la sera, l'ufficio durante le pulizie.
   Compare DOPO le Condizioni, non insieme: due veli al primo avvio si chiudono
   senza leggerli. Prima l'obbligo, poi il consiglio. */
const LOCK_OFF=()=>{try{return localStorage.getItem('pii_lock_reminder_off')==='1';}catch{return false;}};
function showLockReminder(force){
  if(!force&&LOCK_OFF())return;
  try{$('lockOff').checked=false;}catch{}   // riaperto a mano = la spunta riparte da zero
  $('lockOverlay').classList.add('open');
}
function closeLockReminder(){
  try{localStorage.setItem('pii_lock_reminder_off',$('lockOff').checked?'1':'0');}catch{}
  $('lockOverlay').classList.remove('open');
}

/* ---- fork: la sessione si svuota da sola dopo IDLE_MIN minuti -------------
   Il blocco schermo protegge da chi guarda; questo protegge da chi sblocca, e dal
   computer di segreteria lasciato acceso con la pratica aperta. Il timer riparte a
   ogni gesto, con un freno da 5 s perche' un mousemove ne genererebbe centinaia.
   Non scatta mentre l'app lavora: su un PDF lungo si aspetta senza toccare niente,
   e svuotare la sessione a meta' anonimizzazione sarebbe un guasto travestito da
   misura di sicurezza. DOC_TTL lato server (10 min) e' la rete sotto a questo. */
const IDLE_MIN=7;
let IDLE_T=null,IDLE_WARN=null,IDLE_LAST=0;
function idleReset(){
  const now=Date.now();
  if(now-IDLE_LAST<5000)return;
  IDLE_LAST=now;
  clearTimeout(IDLE_T);clearTimeout(IDLE_WARN);
  IDLE_WARN=setTimeout(()=>toast(tt('t_idle_soon'),false,15000),(IDLE_MIN-1)*60000);
  IDLE_T=setTimeout(idleWipe,IDLE_MIN*60000);
}
function idleWipe(){
  if($('go').disabled){idleReset();return;}          // sta lavorando: non si tocca
  wipeSession(true);
  // il pannello Ripristina mostra i valori VERI: e' il primo da svuotare, non l'ultimo
  $('rin').value='';$('rout').innerHTML=routEmpty();$('rout')._raw='';
  $('idleOverlay').classList.add('open');
}
for(const ev of ['pointerdown','keydown','wheel','mousemove','touchstart','input','drop'])
  document.addEventListener(ev,idleReset,{passive:true,capture:true});
idleReset();

/* chiudere la finestra chiude la pratica: i documenti muoiono subito, senza TTL */
window.addEventListener('pagehide',()=>{dropDoc(SRC_DOC);dropDoc(OUT_DOC);});

if(termsAck()!==TERMS_VERSION)$('termsOverlay').classList.add('open');
else showLockReminder(false);
...
if __name__ == "__main__":
    ...
    _p.add_argument("--auth", default=None, metavar="UTENTE:PASSWORD",
                    help="credenziale richiesta a chi apre l'app (serve solo se esponi "
                         "il server in rete; anche da env PII_AUTH)")
    _args = _p.parse_args()

    if _args.auth is not None:
        AUTH = server_config.parse_auth(_args.auth)
        if AUTH is None:
            print("ERRORE: --auth vuole la forma utente:password (utente e password non vuoti)")
            sys.exit(2)
    ...
    # fork: esporre il server senza credenziale e' una scelta, non un incidente — ma
    # va detta ad alta voce. Avviso, non rifiuto: il docker-compose fa girare l'app su
    # 0.0.0.0 DENTRO il container con la porta mappata su 127.0.0.1 dell'host, e li' il
    # confine lo mette gia' Docker. Rifiutare l'avvio spegnerebbe una configurazione
    # corretta; tacere lascerebbe aperta una porta d'ufficio.
    if not server_config.is_loopback(_host):
        if AUTH:
            print(f"Server ESPOSTO su {_host} — accesso con credenziale ({AUTH[0]}).")
        else:
            print(f"ATTENZIONE: server esposto su {_host} SENZA credenziale: chiunque "
                  "raggiunga questa macchina puo' usarlo e i documenti degli altri "
                  "passano da qui.\n"
                  "            Chiudilo con  PII_AUTH=\"utente:password\"  (o --auth), "
                  "e mettici davanti un proxy cifrato se esce dalla rete locale.")
