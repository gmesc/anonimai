...
| nLPD art. 6 (proporzionalità, minimizzazione) | Trattare solo i dati necessari allo scopo | Al fornitore dell'LLM arrivano **solo i segnaposto** (`[FULLNAME_1]`), non i valori; i documenti stanno in RAM (`_DOCS`, LRU 6, **TTL 30 min**) e mai su disco | Non mandare fuori il documento integrale quando basta la versione con segnaposto; rileggere l'output |
| nLPD art. 7 (by design / by default) | Impostazioni predefinite protettive | Server su `127.0.0.1`; nessuna chiamata di rete, telemetria o CDN; dizionario in `sessionStorage` per **default** (muore con la finestra), persistenza solo opt-in; anonimizzazione a **priorità di sicurezza** (errore per eccesso: la parola tagliata si maschera intera) | Non esporre il server (`--host 0.0.0.0`) senza accesso controllato; non attivare «ricorda» su macchine condivise |
| nLPD art. 8 (sicurezza dei dati) | Misure tecniche e organizzative adeguate | Redazione **vera** del PDF (`apply_redactions`, annotazioni/widget sotto la redazione eliminati, metadati/XMP/allegati ripuliti); verifica finale dei **residui** con ri-OCR; `422` se nel PDF non si trova nessuna occorrenza | Disco cifrato; cancellare dizionari e file temporanei a pratica chiusa; backup consapevoli; controllo degli accessi al computer |
...
formato); numeri di registro di commercio nel vecchio formato; qualunque identificabilità indiretta.
...
2. Disco cifrato (FileVault / BitLocker) sulle macchine che fanno girare l'app.
...
7. Server esposto in rete (`--host 0.0.0.0`, Docker): accesso autenticato e cifrato; ricordare il §13 AGPL (offrire la sorgente).
...
dell'informativa e del registro; wording della scheda pubblica e dei termini d'uso dell'app (esclusione
di garanzia, obbligo di rilettura).
...
- Codice: `src/app/detectors_local.py`, `tests/test_svizzera.py`, `tests/fixtures_ticino.jsonl`, `docs/CHANGELOG.md`
